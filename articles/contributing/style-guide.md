...
Se uma operação ou função não for destinada a uso direto, mas sim a utilização de uma chamada correspondente que aja por aplicação parcial, considere utilizar um nome que comece `_` por ser chamado para a chamada que é parcialmente aplicada.

# <a name="guidance"></a>[Orientação](#tab/guidance)

Sugerimos:

- Quando uma função, operação ou tipo definido pelo utilizador não faz parte da API pública para uma biblioteca ou programa Q# certifique-se de que o seu nome começa com um sublinhado principal ( `_` ).
