...
|Standard|Operações e funções definidas em Q# baseando-se na lógica definida no prelúdio. A implementação padrão da biblioteca é agnóstica no que diz respeito às máquinas-alvo.|
...
|Estado Quântico|Uma representação dos qubits no sistema. Isto é geralmente denotado como um vetor de coluna complexa. Para obter mais informações, consulte <xref:microsoft.quantum.concepts.vectors>. |
