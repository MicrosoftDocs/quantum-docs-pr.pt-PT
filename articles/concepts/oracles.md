...
Frequentemente, tais operações são definidas usando uma função clássica $ f: \\ { 0, 1 \\ } ^n \to \\ { 0, 1 \\ } ^m $ que toma uma entrada $ $ binária n-bit e produz uma $ saída $ binária m-bit.
Para tal, considere uma entrada binária específica $ x = (x_ { 0 } , x_ { 1 , } \pontos, x_ { n-1 } ) $ .
Podemos rotular os estados qubit como $ \ket { \vec { x } } = \ket { x_ { 0 } } \otimes \ket { x_ { 1 } } \otimes \cdots \otimes \ket { x_ { n-1 } } $ .

Podemos primeiro tentar definir O para $ $ que O x $ \ket { } = \ket { f(x), mas isto tem } $ alguns problemas.
Em primeiro lugar, $ f pode ter um tamanho diferente de entrada e saída $ $ (n m ), de modo que \ne $ a aplicação de O $ $ alteraria o número de qubits no registo.
Em segundo lugar, mesmo $ que n m , a = $ função pode não ser ververtível: se $ f(x) = f(y) $ para alguns x $ \ne y , em seguida O $ x O $ \ket { } = \ket { y mas } $ $ O^ O \dagger x \ket { } \ne O^ \dagger O \ket { y } $ .
...
Em seguida, definiremos o efeito do oráculo em todos os estados computacionais: para todos $ x \in \\ { 0, 1 \\ } ^n $ e $ y \in \\ { 0, \\ } 1^m, $

$$
\begin{align}
    O( \ket { x } \otimes \ket { y } ) x = \ket { } \otimes \ket { y \oplus f(x) } .
\end{align}
$$
...
Se quisermos saber como $ H $ $ \ket { + } $ age, podemos usar que $ H é $ linear,

$$
\begin{align}
H \ket { + } & = \frac { 1 } { \sqrt { 2 } } H( \ket { 0 }  +  \ket { 1 } ) = \frac { 1 } { \sqrt { 2 } } (H \ket { 0 } + H \ket { 1 } )\\\\
           &= \frac{ 1 } { \sqrt { 2 } } ( \ket { + }  +  \ket { - } ) = \frac 12 ( \ket { }  +  \ket { 0 1 }  +  \ket { }  -  \ket { 0 1 } ) = \ket { 0 } .
\end{align}
$$
