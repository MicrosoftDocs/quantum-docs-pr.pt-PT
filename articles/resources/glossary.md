...
Um dos quatro [estados quânticos](xref:microsoft.quantum.glossary#quantum-state) maximicamente [emaranhados](xref:microsoft.quantum.glossary#entanglement) de dois qubits. Os quatro estados são definidos $ \ket { \beta _ { ij } } = \mathbb { (I } \otimes X^iZ^j) ( \ket { 00 }  +  \ket { 11 } ) / \sqrt { 2 } $ . Um estado bell também é conhecido como um [par de EPR](xref:microsoft.quantum.glossary#epr-pair).
...
Para mais informações, consulte [ Q# os programas](xref:microsoft.quantum.guide.programs)
...
Uma [operação](xref:microsoft.quantum.glossary#operation) quântica que leva um ou mais [qubits](xref:microsoft.quantum.glossary#qubit) como facilitadores para a operação alvo. Para mais informações, consulte [a aplicação Functor.](xref:microsoft.quantum.qsharp.functorapplication#functor-application)

## <a name="dirac-notation"></a>Notação dirac

Uma abreviatura simbólica que simplifica a representação dos [Estados quânticos,](xref:microsoft.quantum.glossary#quantum-state)também chamada de notação *bra-ket.*  A porção de *soutien* representa um vetor de linha, por exemplo $ \bra { } = \begin{bmatrix} A { _1 } & A { _2 } \end{bmatrix} $ e a porção de *ket* representa um vetor de coluna, $ \ket { B } = \begin{bmatrix} { _1 } \\\\ B { _2 } \end{bmatrix} $ . Para mais informações, consulte [a Notação De Dirac.](xref:microsoft.quantum.concepts.dirac)

## <a name="eigenvalue"></a>Rio Eigenvalue

O fator pelo qual a magnitude de um [eigenvector](xref:microsoft.quantum.glossary#eigenvector) de uma determinada transformação é alterada pela aplicação da transformação.  Dada a matriz quadrada $ M $ e um eigenvector $ $ v, então $ Mv = $ cv, onde c é $ o $ eigenvalue e pode ser um número complexo de qualquer argumento. Para mais informações, consulte [conceitos de matriz avançada.](xref:microsoft.quantum.concepts.matrix-advanced)

## <a name="eigenvector"></a>Eigenvector

Um vetor cuja direção é inalterada por uma determinada transformação e cuja magnitude é alterada por um fator correspondente ao [eigenvalue](xref:microsoft.quantum.glossary#eigenvalue)desse vetor. Dada a matriz quadrada $ M $ e um eigenvalue $ $ c, então $ Mv = $ cv, onde v é um $ $ eigenvector da matriz e pode ser um número complexo de qualquer argumento. Para mais informações, consulte [conceitos de matriz avançada.](xref:microsoft.quantum.concepts.matrix-advanced)
...
Um tipo de subrotina na Q# linguagem que é puramente determinista. Embora as funções sejam utilizadas dentro de algoritmos quânticos, não podem agir em [qubits](xref:microsoft.quantum.glossary#qubit) ou [operações](xref:microsoft.quantum.glossary#operation)de chamada . Para mais informações, consulte [ Q# os programas](xref:microsoft.quantum.guide.programs)

## <a name="gate"></a>Portão

Um termo legado para certas [operações](xref:microsoft.quantum.glossary#operation)quânticas intrínsecas, baseadas no conceito de portas lógicas clássicas. Um [circuito quântico](xref:microsoft.quantum.glossary#quantum-circuit-diagram) é uma rede de portões, baseada no conceito semelhante de circuitos lógicos clássicos.

## <a name="global-phase"></a>Fase global

Quando dois [estados](xref:microsoft.quantum.glossary#quantum-state) são idênticos até um múltiplo de um número complexo $ e^ { \phi } $ i, dizem que diferem até uma fase global. Ao contrário das fases locais, as fases globais não podem ser observadas através de qualquer [medição](xref:microsoft.quantum.glossary#measurement). Para mais informações, consulte [o Qubit.](xref:microsoft.quantum.concepts.qubit)
...
Uma variável cujo valor não pode ser alterado. Uma variável imutável Q# é criada usando a `let` palavra-chave. Para declarar variáveis que *podem* ser alteradas, use a palavra-chave [mutável](xref:microsoft.quantum.glossary#immutable) para declarar e a `set` palavra-chave para modificar o valor. 
...
A unidade básica da computação quântica em Q# . É aproximadamente equivalente a uma função em C, C++ ou Python, ou um método estático em C# ou Java. Para mais informações, consulte [ Q# os programas.](xref:microsoft.quantum.guide.programs)
...
Convocar uma [função](xref:microsoft.quantum.glossary#function) ou [funcionamento](xref:microsoft.quantum.glossary#operation) sem todas as entradas necessárias. Isto devolve uma nova [chamada](xref:microsoft.quantum.glossary#callable) que só necessita dos parâmetros em falta (indicados por um sublinhado) a serem fornecidos durante uma futura aplicação. Para mais informações, consulte [aplicação parcial.](xref:microsoft.quantum.qsharp.partialapplication)

## <a name="pauli-operators"></a>Operadores Pauli

Um conjunto de três matrizes unitárias de 2 x 2 conhecidas como `X` `Y` `Z` operações quânticas. A matriz identitária, $ $ eu, é frequentemente incluída no conjunto também.  $I = \begin{bmatrix} 1 & 0 \\\\ 0 & 1 \end{bmatrix} $ , X $ = \begin{bmatrix} 0 & 1 \\\\ & 10 , \end{bmatrix} $ $ Y = \begin{bmatrix} 0 & -i \\\\ & 0 , Z \end{bmatrix} $ $ = \begin{bmatrix} 1 & \\\\ 0 0 & -1 \end{bmatrix} $ .   Para obter mais informações, consulte [as operações Single-qubit](xref:microsoft.quantum.concepts.qubit#single-qubit-operations).

## <a name="quantum-circuit-diagram"></a>Diagrama do circuito quântico

Um método para representar graficamente a sequência de [portas](xref:microsoft.quantum.glossary#gate) para programas quânticos simples, por exemplo 
...
Um conceito frequentemente usado em algoritmos quânticos que consiste em aplicar repetidamente uma computação até que uma determinada condição seja satisfeita. Quando a condição não está satisfeita, muitas vezes é necessária uma correção antes de voltar a tentar, introduzindo a iteração seguinte. Para mais informações, consulte o guia do [ Q# utilizador](xref:microsoft.quantum.guide)

## <a name="standard-libraries"></a>Bibliotecas padrão

[Operações](xref:microsoft.quantum.glossary#operation), [funções](xref:microsoft.quantum.glossary#function) e [tipos definidos pelo utilizador](xref:microsoft.quantum.glossary#user-defined-type) que são instalados juntamente com o compilador durante a Q# instalação. A implementação padrão da biblioteca é agnóstica no que diz respeito às máquinas-alvo. Para mais informações, consulte [as bibliotecas Standard.](xref:microsoft.quantum.libraries.standard.intro)
...
Uma coleção de valores separados por vírgula que funciona como um único valor. O *tipo* de tuple é definido pelos tipos de valores que contém. Em Q# , tuples são [imutáveis](xref:microsoft.quantum.glossary#immutable) e podem ser aninhados, conter matrizes, ou usados em uma matriz. Para mais informações, consulte [Tuples.](xref:microsoft.quantum.qsharp.valueliterals#tuple-literals)

## <a name="unitary-operator"></a>Operador unitário

Um operador cujo inverso é igual ao seu [adjacente,](xref:microsoft.quantum.glossary#adjoint)isto é, $ UU^ { \dagger } = \id $ .

## <a name="user-defined-type"></a>Tipo definido pelo utilizador

Um tipo personalizado que pode conter um ou mais itens nomeados ou anónimos. Para obter mais informações, consulte [Declarações tipo]microsoft.quantum.qsharp.typedeclarations#type-declarations).
