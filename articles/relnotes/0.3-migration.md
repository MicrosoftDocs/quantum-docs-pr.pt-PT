...
Uma expressão condicional é uma expressão da `condition ? caseTrue | caseFalse` forma.
Como se pode ver pelo exemplo acima, a iteração sobre as matrizes é agora também apoiada.

Atribuições no interior `using` e `borrowing` são recentemente possíveis para qubits individuais, matrizes de qubit e tuples aninhados dos mesmos:
```qsharp
    using (qubit = Qubit()) {
...
São introduzidas duas novas diretivas relativas aos geradores de especialização e, para `invert` `distribute` além disso, introduzidas para aumentar o controlo sobre a `auto` forma como as especializações de functores são geradas.
Uma declaração explícita da especialização por defeito no âmbito das `body` operações já não é obrigatória.
...
`Adjoint`, `Controlled` e um novo operador de pós-fixação "desembrulhar" é tratado como modificadores de `!` expressão. Tratar `Adjoint` e como modificadores de expressão elimina a necessidade de `Controlled` parênteses para muitos casos. As expressões abaixo, por exemplo, são válidas `Op` para, uma operação unitária, `opArr` um conjunto de operações unitárias e `arg` um argumento adequado: 
```qsharp
    Adjoint Op (arg);
    Controlled opArr[i] (arg);
```
O roteiro de migração vai deixar cair a maior parte dos parênteses desnecessários e tirar o máximo partido das novas funcionalidades. No entanto, esteja avisado de que o script não interpreta o código, e não terá em conta a interpretação diferente do `Controlled` functor em casos raros (apenas relevante em combinação com aplicações parciais).
...
```
Como se pode ver pelo exemplo, `!` liga-se mais apertado do que os modificadores prefixos, como `Controlled` e `Adjoint` .

## <a name="breaking-changes"></a><a name="BreakingChanges"></a>Alterações de rutura 
...
Além das alterações listadas abaixo, o padrão `__*__` em que * é qualquer sequência de caracteres não-espaciais é reservado para uso interno e não pode ser usado como um nome de símbolo.  
...
O exemplo que se segue ilustra como utilizar a `ToOperation` função para alterar uma função `Square` para uma `op` operação: 
```qsharp
...
## <a name="migrating-existing-projects-to-q-03"></a><a name="Migration"></a>Migração de projetos existentes para o Q#0.3 
...
2. Num editor de texto, edite o ficheiro .csproj para alterar a versão de todos os "Microsoft.Quantum" `PackageReference` para a versão 0.3.1811.2802-preview, por exemplo:
```xml
...
4. A partir da linha de comando, utilize a ferramenta de formatação integrada no compilador de linha de comando para abordar todas as alterações de sintaxe executando este comando:`dotnet msbuild /t:qsharpformat`  
    - Os seus ficheiros serão migrados no local. Uma cópia de segurança de todos os ficheiros originais`obj\qsharp\.backup`
...
## <a name="migration-script"></a><a name="MigrationScript"></a>Roteiro de migração 
...
- O projeto é construído com a versão anterior do Kit de Desenvolvimento Quântico para garantir que a migração é suscetível de ser bem sucedida. Este passo pode ser ignorado utilizando o `-Force` parâmetro para o script.
...
> Para permitir scripts como `qdk-migrate.ps1` executar no Windows, poderá ter de alterar a política de execução.
> Para isso, dirija o `Set-ExecutionPolicy` comando:
> ```powershell
> Set-ExecutionPolicy RemoteSigned -Scope Process
> ```
> A política de execução será revertida quando sair da PowerShell.
> Se quiser salvar a política de execução, utilize um valor diferente `-Scope` para:
> ```powershell
