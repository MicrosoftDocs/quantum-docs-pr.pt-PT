...
Depois de realizar um teste no Test Explorer e clicar no teste, um painel apresenta informações sobre o teste: Estado de passagem/falha, tempo decorrido e uma ligação à saída. Clique em _ *Saída** para abrir a saída do teste numa nova janela.
...
_Assertions*, por outro lado, são usados de forma semelhante aos factos, mas podem depender do estado da máquina-alvo. Consequentemente, são definidas como operações, enquanto os factos são definidos como funções (como no exemplo anterior).
