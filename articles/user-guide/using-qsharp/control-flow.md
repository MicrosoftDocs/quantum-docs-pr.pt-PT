...
Adiamos a discussão sobre este último para [mais abaixo.](#repeat-until-success-loop)
No `if` entanto, o fluxo de controlo e o controlo procedem num sentido familiar à maioria das línguas clássicas de `for` programação.

Importante, `for` os loops e `if` as declarações podem até ser usados em operações para as quais as especializações são geradas automaticamente. Nesse caso, a adjoint de um `for` laço inverte a direção e toma a adjoint de cada iteração.
...
## <a name="if-else-if-else"></a>Se, Se, Se, Else

A declaração apoia a `if` execução condicional.
...
Isto pode ser seguido por qualquer número de cláusulas se, cada uma das quais consiste na palavra-chave `elif` , um parêntese `(` aberto, uma expressão booleana, um parêntese próximo, e um bloco de `)` declaração (o bloco de outra _si)._
Finalmente, a declaração pode terminar opcionalmente com uma outra cláusula, que consiste na palavra-chave seguida de outro bloco de `else` declaração (o _bloco de outra_ si).

A `if` condição é avaliada, e se for verdade, o bloco de então é executado.
...
A `for` declaração suporta a iteração sobre uma gama inteiro ou sobre uma matriz.
A declaração consiste na palavra-chave `for` , um parêntese `(` aberto, seguida de um símbolo ou símbolo, a palavra-chave, `in` uma expressão de tipo ou `Range` matriz, um parêntese `)` próximo, e um bloco de declaração.
...
A ligação dos símbolos declarados é imutável e segue as mesmas regras que outras encadernações variáveis. 

Para alguns exemplos, supondo `qubits` é um registo de qubits (isto é, de `Qubit[]` tipo), 
...
```
Note que no final utilizamos o operador binário aritmético-à esquerda, `<<<` cujos detalhes podem ser encontrados em [Expressões Numéricas](xref:microsoft.quantum.guide.expressions#numeric-expressions)
...
onde `expression` é qualquer expressão válida que avalie um valor de `Bool` tipo.
...
As três porções de um laço repetido/até ao ciclo (o corpo, o teste e a fixação) são tratadas como uma única margem *para cada repetição,* pelo que os símbolos que estão ligados no corpo estão disponíveis no teste e na fixação.
No entanto, a conclusão da execução da correção termina o âmbito da declaração, de modo a que as ligações de símbolos efetuadas durante o corpo ou a fixação não estejam disponíveis em repetições subsequentes.

Além disso, a `fixup` afirmação é muitas vezes útil, mas nem sempre necessária.
...
Q# apoia, portanto, a utilização de laços enquanto as funções apenas. Uma `while` declaração consiste na palavra-chave , um `while` parêntese `(` aberto, uma condição (isto é, uma expressão booleana), um parêntese `)` próximo, e um bloco de declaração.
O bloco de declaração (o corpo do laço) é executado desde que a condição avalie `true` para .
...
Se desejar uma saída antecipada, `return ()` pode ser utilizado neste caso.
...
Consiste na palavra-chave, `fail` seguida de uma corda e de um ponto evíomina.
...
```
ou, usando [cordas interpoladas,](xref:microsoft.quantum.guide.expressions#interpolated-strings)
```qsharp
...
Por exemplo, o seguinte código é um circuito probabilístico que implementa um importante portão de rotação $V_3 = (\boldone + 2 i Z) / \sqrt {5} $ usando os `H` portões e `T` portões.
O loop termina em repetições de {8} {5} $\frac $ em média.
Ver [*Repeat-Until-Success: Decomposição não-determinística de unitários de um único qubit*](https://arxiv.org/abs/1311.1074) (Paetznick e Svore, 2014) para mais detalhes.
...
Finalmente, mostramos um exemplo de um padrão RUS para preparar um estado quântico $\frac {1} {\sqrt {3} }\left (\sqrt {2} {0} \ket {1} \ket \ket \-right),a partir do estado $\ket{}}$.
...
Características programáticas notáveis mostradas nesta operação são uma parte mais complexa `fixup` do loop, que envolve operações quânticas, e o uso de `AssertProb` declarações para apurar a probabilidade de medir o estado quântico em certos pontos especificados no programa.
Consulte também [testes e depuração](xref:microsoft.quantum.guide.testingdebugging) para obter mais informações sobre as [`Assert`](xref:microsoft.quantum.intrinsic.assert) operações e [`AssertProb`](xref:microsoft.quantum.intrinsic.assertprob) operações.
