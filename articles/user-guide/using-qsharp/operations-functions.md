---
title: 'Operações e funções em :::no-loc(Q#):::'
...
---
# <a name="operations-and-functions-in-no-locq"></a><span data-ttu-id="f4d19-103">Operações e Funções em :::no-loc(Q#):::</span><span class="sxs-lookup"><span data-stu-id="f4d19-103">Operations and Functions in :::no-loc(Q#):::</span></span>
...
<span data-ttu-id="f4d19-105">As operações são o núcleo :::no-loc(Q#)::: de.</span><span class="sxs-lookup"><span data-stu-id="f4d19-105">Operations are the core of :::no-loc(Q#):::.</span></span>
<span data-ttu-id="f4d19-106">Uma vez declarados, podem ser chamados a partir de aplicações clássicas .NET, por exemplo, utilizando um simulador ou por outras operações no seu interior :::no-loc(Q#)::: .</span><span class="sxs-lookup"><span data-stu-id="f4d19-106">Once declared, they can either be called from classical .NET applications, for example, using a simulator or by other operations within :::no-loc(Q#):::.</span></span>
<span data-ttu-id="f4d19-107">Cada operação definida :::no-loc(Q#)::: pode chamar qualquer número de outras operações, incluindo as operações intrínsecas incorporadas definidas pela língua.</span><span class="sxs-lookup"><span data-stu-id="f4d19-107">Each operation defined in :::no-loc(Q#)::: can call any number of other operations, including the built-in intrinsic operations defined by the language.</span></span> <span data-ttu-id="f4d19-108">A forma particular como :::no-loc(Q#)::: define estas operações intrínsecas depende da máquina-alvo.</span><span class="sxs-lookup"><span data-stu-id="f4d19-108">The particular way in which :::no-loc(Q#)::: defines these intrinsic operations depends on the target machine.</span></span>
...
<span data-ttu-id="f4d19-110">Cada :::no-loc(Q#)::: ficheiro de origem pode definir qualquer número de operações.</span><span class="sxs-lookup"><span data-stu-id="f4d19-110">Each :::no-loc(Q#)::: source file can define any number of operations.</span></span>
...
> [!NOTE]
> <span data-ttu-id="f4d19-124">Cada operação :::no-loc(Q#)::: requer exatamente uma entrada e devolve exatamente uma saída.</span><span class="sxs-lookup"><span data-stu-id="f4d19-124">Each operation in :::no-loc(Q#)::: takes exactly one input and returns exactly one output.</span></span>
...
> <span data-ttu-id="f4d19-126">A este respeito, :::no-loc(Q#)::: é uma linguagem "tuple-in tuple-out".</span><span class="sxs-lookup"><span data-stu-id="f4d19-126">In this regard, :::no-loc(Q#)::: is a "tuple-in tuple-out" language.</span></span>
...
<span data-ttu-id="f4d19-142">Um *functor* :::no-loc(Q#)::: é uma fábrica que define uma nova operação a partir de outra operação.</span><span class="sxs-lookup"><span data-stu-id="f4d19-142">A *functor* in :::no-loc(Q#)::: is a factory that defines a new operation from another operation.</span></span>
<span data-ttu-id="f4d19-143">Os dois funtores padrão :::no-loc(Q#)::: são `Adjoint` `Controlled` e.</span><span class="sxs-lookup"><span data-stu-id="f4d19-143">The two standard functors in :::no-loc(Q#)::: are `Adjoint` and `Controlled`.</span></span>
...
> [!NOTE]
> <span data-ttu-id="f4d19-159">Em :::no-loc(Q#)::: , versões controladas sempre tomam uma série de qubits de controlo, e o controlo é sempre baseado em todos os qubits de controlo que estão no estado computacional `PauliZ` , `One` $\ket {1} $.</span><span class="sxs-lookup"><span data-stu-id="f4d19-159">In :::no-loc(Q#):::, controlled versions always take an array of control qubits, and the controlling is always based on all of the control qubits being in the computational (`PauliZ`) `One` state, $\ket{1}$.</span></span>
...
<span data-ttu-id="f4d19-180">Esta secção detalha como incluir a existência de especializações nas suas declarações de :::no-loc(Q#)::: operação, dando-lhes assim a capacidade de chamar em conjunto com os `Adjoint` ou `Controlled` funtores.</span><span class="sxs-lookup"><span data-stu-id="f4d19-180">This section details how to include the existence of specializations in your :::no-loc(Q#)::: operation declarations, hence giving them the ability to called in conjunction with the `Adjoint` or `Controlled` functors.</span></span>
...
<span data-ttu-id="f4d19-201">:::no-loc(Q#)::: As operações podem conter as seguintes declarações explícitas de especialização:</span><span class="sxs-lookup"><span data-stu-id="f4d19-201">:::no-loc(Q#)::: operations can contain the following explicit specialization declarations:</span></span>
...
<span data-ttu-id="f4d19-227">Uma declaração de especialização contendo uma implementação definida pelo utilizador consiste num tuple de argumento seguido de um bloco de declaração com o :::no-loc(Q#)::: código que implementa a especialização.</span><span class="sxs-lookup"><span data-stu-id="f4d19-227">A specialization declaration containing a user-defined implementation consists of an argument tuple followed by a statement block with the :::no-loc(Q#)::: code that implements the specialization.</span></span>
...
<span data-ttu-id="f4d19-257">:::no-loc(Q#)::: é *covariante* no que diz respeito aos tipos de retorno callable: uma chamada que devolve um tipo `'A` é compatível com uma chamada com o mesmo tipo de entrada e um tipo de resultado que é compatível com `'A` .</span><span class="sxs-lookup"><span data-stu-id="f4d19-257">:::no-loc(Q#)::: is *covariant* with respect to callable return types: a callable that returns a type `'A` is compatible with a callable with the same input type and a result type that is compatible with `'A` .</span></span>

<span data-ttu-id="f4d19-258">:::no-loc(Q#)::: é *contravariante* no que diz respeito aos tipos de entrada: uma chamada que toma um tipo `'A` como entrada é compatível com uma chamada com o mesmo tipo de resultado e um tipo de entrada compatível com `'A` .</span><span class="sxs-lookup"><span data-stu-id="f4d19-258">:::no-loc(Q#)::: is *contravariant* with respect to input types: a callable that takes a type `'A` as input is compatible with a callable with the same result type and an input type that is compatible with `'A`.</span></span>
...
> [!IMPORTANT]
> <span data-ttu-id="f4d19-264">:::no-loc(Q#)::: 0.3 introduziu uma diferença significativa no comportamento dos tipos definidos pelo utilizador.</span><span class="sxs-lookup"><span data-stu-id="f4d19-264">:::no-loc(Q#)::: 0.3 introduced a significant difference in the behavior of user-defined types.</span></span>
...
<span data-ttu-id="f4d19-270">Como consequência, :::no-loc(Q#)::: as funções são *puras,* na medida em que mapeiam sempre os mesmos valores de entrada para os mesmos valores de saída.</span><span class="sxs-lookup"><span data-stu-id="f4d19-270">As a consequence, :::no-loc(Q#)::: functions are *pure* , in that they always map the same input values to the same output values.</span></span>
<span data-ttu-id="f4d19-271">Este comportamento permite ao :::no-loc(Q#)::: compilador reencomendar com segurança como e quando chamar funções ao gerar especializações de operação.</span><span class="sxs-lookup"><span data-stu-id="f4d19-271">This behavior allows the :::no-loc(Q#)::: compiler to safely reorder how and when to call functions when generating operation specializations.</span></span>

<span data-ttu-id="f4d19-272">Cada :::no-loc(Q#)::: ficheiro de origem pode definir qualquer número de funções.</span><span class="sxs-lookup"><span data-stu-id="f4d19-272">Each :::no-loc(Q#)::: source file can define any number of functions.</span></span>
...
<span data-ttu-id="f4d19-280">Para sublinhar a diferença entre funções e operações, considere o problema da amostragem clássica de um número aleatório de dentro de uma :::no-loc(Q#)::: operação:</span><span class="sxs-lookup"><span data-stu-id="f4d19-280">To underscore the difference between functions and operations, consider the problem of classically sampling a random number from within a :::no-loc(Q#)::: operation:</span></span>
...
<span data-ttu-id="f4d19-289">Para implementar isto :::no-loc(Q#)::: em , aproveite o facto de que as funções são de primeira classe.</span><span class="sxs-lookup"><span data-stu-id="f4d19-289">To implement this in :::no-loc(Q#):::, take advantage of the fact that functions are first class.</span></span>
...
<span data-ttu-id="f4d19-298">Efetivamente, pretende-se que o compilador trate `Map` como uma espécie de função matemática, desde :::no-loc(Q#)::: *tipos* a :::no-loc(Q#)::: funções.</span><span class="sxs-lookup"><span data-stu-id="f4d19-298">Effectively, you want the compiler to treat `Map` as some kind of mathematical function from :::no-loc(Q#)::: *types* to :::no-loc(Q#)::: functions.</span></span>

<span data-ttu-id="f4d19-299">:::no-loc(Q#)::: formaliza esta noção permitindo que funções e operações tenham *parâmetros de tipo,* bem como os seus parâmetros comuns de tuple.</span><span class="sxs-lookup"><span data-stu-id="f4d19-299">:::no-loc(Q#)::: formalizes this notion by allowing functions and operations to have *type parameters* , as well as their ordinary tuple parameters.</span></span>
...
<span data-ttu-id="f4d19-305">Em :::no-loc(Q#)::: , isto é escrito adicionando suportes de ângulo (isto `<>` é, não travões $\braket {} $!) após o nome de uma função ou operação na sua declaração, e enumerando cada parâmetro do tipo.</span><span class="sxs-lookup"><span data-stu-id="f4d19-305">In :::no-loc(Q#):::, this is written by adding angle brackets (that's `<>`, not brakets $\braket{}$!) after the name of a function or operation in its declaration, and by listing each type parameter.</span></span>
...
> [!TIP]
> <span data-ttu-id="f4d19-311">Escrever funções genéricas e operações é um lugar onde "tuple-in tuple-out" é uma forma muito útil de pensar sobre :::no-loc(Q#)::: funções e operações.</span><span class="sxs-lookup"><span data-stu-id="f4d19-311">Writing generic functions and operations is one place where "tuple-in tuple-out" is a very useful way to think about :::no-loc(Q#)::: functions and operations.</span></span>
> <span data-ttu-id="f4d19-312">Uma vez que cada função requer exatamente uma entrada e devolve exatamente uma saída, uma entrada de tipo `'T -> 'U` corresponde a *qualquer* :::no-loc(Q#)::: função.</span><span class="sxs-lookup"><span data-stu-id="f4d19-312">Since every function takes exactly one input and returns exactly one output, an input of type `'T -> 'U` matches *any* :::no-loc(Q#)::: function whatsoever.</span></span>
...
<span data-ttu-id="f4d19-318">As :::no-loc(Q#)::: bibliotecas-padrão fornecem uma gama de operações e funções por tipo-parametrizadas para facilitar a expressão do fluxo de controlo de ordem superior.</span><span class="sxs-lookup"><span data-stu-id="f4d19-318">The :::no-loc(Q#)::: standard libraries provide a range of such type-parameterized operations and functions to make higher-order control flow easier to express.</span></span>
<span data-ttu-id="f4d19-319">Estes são discutidos mais adiante no [ :::no-loc(Q#)::: guia padrão](xref:microsoft.quantum.libraries.standard.intro)da biblioteca.</span><span class="sxs-lookup"><span data-stu-id="f4d19-319">These are discussed further in the [:::no-loc(Q#)::: standard library guide](xref:microsoft.quantum.libraries.standard.intro).</span></span>
...
<span data-ttu-id="f4d19-323">Por exemplo, o seguinte é um código perfeitamente :::no-loc(Q#)::: válido, se um pouco indireto:</span><span class="sxs-lookup"><span data-stu-id="f4d19-323">For instance, the following is perfectly valid :::no-loc(Q#)::: code, if a little indirect:</span></span>
...
<span data-ttu-id="f4d19-344">:::no-loc(Q#)::: as calções podem ser direta ou indiretamente recursivas.</span><span class="sxs-lookup"><span data-stu-id="f4d19-344">:::no-loc(Q#)::: callables are allowed to be directly or indirectly recursive.</span></span>
...
- <span data-ttu-id="f4d19-349">Ao correr num dispositivo quântico real, o espaço da pilha pode ser limitado, e assim a recursão profunda pode levar a um erro de tempo de execução.</span><span class="sxs-lookup"><span data-stu-id="f4d19-349">When running on an actual quantum device, stack space might be limited, and so deep recursion can lead to a runtime error.</span></span>
  <span data-ttu-id="f4d19-350">Em particular, o compilador e o :::no-loc(Q#)::: tempo de execução não identificam e otimizam a recursão da cauda.</span><span class="sxs-lookup"><span data-stu-id="f4d19-350">In particular, the :::no-loc(Q#)::: compiler and runtime do not identify and optimize tail recursion.</span></span>
...
<span data-ttu-id="f4d19-352">Saiba mais sobre [variáveis](xref:microsoft.quantum.guide.variables) em :::no-loc(Q#)::: .</span><span class="sxs-lookup"><span data-stu-id="f4d19-352">Learn about [Variables](xref:microsoft.quantum.guide.variables) in :::no-loc(Q#):::.</span></span>